...
const calculateBasePrice = (
  assetBtcPrice: string | number,
  usdPrice: string | number
) => (Number(assetBtcPrice) * Number(usdPrice)).toString();

export const filterCryptoBase = (
  exchanges: IMarketAsset[] | any,
  usdtExchanges: IMarketAsset[],
  usdExchanges: IMarketAsset[]
) => {
...
export const extractCryptoMarkets = (
  currency: string,
  { marketBTC, marketETH, marketUSD, marketUSDT }: IGetMarketsRes
) => {
...
export const extractExchangePrice = (asset: IAsset, markets: IGetMarketsRes) => {
  ...
  const useExchange = (base: string) => {
    switch (base) {
      case 'USD': case 'USDT': return markets['market'+base];
      case 'BTC': return btcMarkets;
      case 'ETH': return ethMarkets;
    }
  }
